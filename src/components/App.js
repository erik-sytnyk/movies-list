...
class App extends Component {
  ...
  componentDidMount() {
    moviesService.getGenres().then(genres => {
      this.setState(
        {
          genres
        },
  ...
  loadData() {
    moviesService.getMovies(this.state.activePage, this.state.sortBy, this.state.searchStr).then(data => {
      this.setState({
  ...
  deleteMovie() {
    moviesService.deleteMovie(this.state.movieToDeleteId).then(() => {
  ...
  saveMovie() {
    ...
    this.setState({movieToEdit: null}, () => {
      moviesService.saveMovie(movie).then(() => {
  ...
  renderMoviesList() {
    if (!this.anyMovies) return <div style={{marginTop: 30}}>No movies.</div>;
    ...
    return (
      <div>
        <div id="movie-list">{this.state.movies.map(movie => this.renderMovie(movie))}</div>
        <EditMovie
          visible={editMovieVisible}
          movie={this.state.movieToEdit}
          genres={this.state.genres}
          save={this.saveMovie}
          close={this.cancelEditMovie}
            ...
            <DropdownButton bsSize="small" title="Sort By:" id="sort-by-dropdown">
              {sortByOptions.map(item => {
                return (
                  <MenuItem
                    key={item.key}
                    onClick={() => this.sortBy(item.key)}
                    active={this.state.sortBy === item.key}>
                    {item.text}
                  </MenuItem>
                );
              })}
  ...
  renderMovie(movie) {
    ...
    return (
      <div className="movie-row" key={movie.id}>
        ...
        <div className="title">
          <h3>
            <a onClick={() => this.updateMovie(movie)} style={{cursor: 'pointer'}}>
              {movie.title}
            </a>
            <Button onClick={() => this.confirmDeleteMovie(movie.id)}>
